...
<div class="abstract">
	<h2>Abstract</h2>
	<p>
		This almost class-less CSS library turns your HTML document into a website that looks like a
		<span class="latex">L<span>a</span>T<span>e</span>X</span>
		document. Write semantic HTML, add
		<code class="language-html">&lt;link rel="stylesheet" href="https://latex.now.sh/style.css"&gt;</code>
		to the <code class="language-html">&lt;head&gt;</code> of your project and you are good to go. The source code can be found on GitHub at
	...
	<article>
		...
		<ul>
			<li>
				Add
				<code class="language-html">&lt;link rel="stylesheet" href="https://latex.now.sh/style.css"&gt;</code>
				to the <code class="language-html">&lt;head&gt;</code> of your website or install the package using <code>npm install latex.css</code>.
			</li>
			<li>
			...
			<li>
				(optional) Add dark mode by adding the <code>latex-dark-auto</code> class to the <code class="language-html">&lt;body&gt;</code>. More infos <a href="#dark-mode">here</a>.
			</li>
			<li>
				(optional) If you need support for
				<span class="latex">L<span>a</span>T<span>e</span>X</span> math, add the following script to include
			...
			<li>
				(optional) If you need syntax highlighting, add the following script to include
				<a href="https://prismjs.com/">Prism</a> and the Prism LaTeX stylesheet (or use any other):
				<pre><code class="language-html"
						>&lt;link rel="stylesheet" href="https://latex.now.sh/prism/prism.css"&gt;

&lt;script src="https://cdn.jsdelivr.net/npm/prismjs/prism.min.js"&gt;&lt;/script&gt;</code
					></pre>
			</li>
			<li>Done.</li>
		</ul>
		...
		<p>
			Use the following code to add an author and abstract to your document. It will look like <a href="#top">this</a>.
		</p>
		<pre><code class="language-html"
...
&lt;div class="proof"&gt;...&lt;/div&gt;</code
		...
		<div class="theorem">The real numbers $\mathbb $ are uncountable.</div>

		<div class="theorem">
			If $S$ is both countable and infinite, then there is a bijection between $S$ and $\boldsymbol $ itself.<sup><a href="#fn1" id="ref1">1</a></sup>
		</div>
		<div class="proof">For any $s \in S,$ we let $f(s)$ denote the value of $k$ such that $s$ is the $k$ the smallest element of $S .$ This map is well defined for any $s,$ because there are only finitely many natural numbers between 1 and $s .$ It is impossible for two different elements of $S$ to both be the $k$ the smallest element of $S$. Hence $f$ is one-to-one. Also, since $S$ is infinite, $f$ is onto.</div>
		<h4 id="lemmas">Lemmas</h4>
		<div class="lemma">An even number plus an even number results in an even number.</div>
		<h4 id="definitions">Definitions</h4>
		<div class="definition">
			A definition is a a statement of the meaning of a word or word group or a sign or symbol.<sup><a href="#fn2" id="ref2">2</a></sup>
		</div>
		...
		<p>
			...
			by including the following snippet, linking to the desired language in addition to the main CSS file.
		</p>
		<pre><code class="language-html"
...
&lt;html lang="es"&gt;</code
			></pre>
		<p>
			Take a peek at the <a href="/languages">language support demo</a> to see how the labels of the different languages change.
		</p>

		<h2 id="sidenotes">Sidenotes</h2>
		<p>
			Sidenotes can be used as an alternative to footnotes, where the user does not have to jump to the bottom of the page to read it. On mobile, click the superscript to reveal the note<label for="sn-1" class="sidenote-toggle sidenote-number" /><input type="checkbox" id="sn-1" class="sidenote-toggle" /><span class="sidenote">Yay, sidenotes!. If you are on mobile, I will appear inline. If you are using a larger screen, the sidenote will appear to the right of the text.</span>.
		</p>

		<p>
			Sidenotes do need a little bit of setup, they are made up of a label, an invisible checkbox on top of the number and a span with the text inside. The superscript is set automatically and incremented using CSS when the checkbox has a class of <code>sidenote-number</code>.
		</p>
		<pre><code class="language-html"
				>&lt;label for="sn-1" class="sidenote-toggle sidenote-number"&gt;&lt;/label&gt;
&lt;input type="checkbox" id="sn-1" class="sidenote-toggle" /&gt;
&lt;span class="sidenote"&gt;&lt;!-- sidenote content --&gt;&lt;/span&gt;</code
			></pre>
		<p>If you do not need superscripted numbers, you can opt out of the <code>sidenote-number</code> class and the sidenote will not have a number assigned. On a smaller screen, you will need to add a symbol inside the <code>label</code> for the user to click on. <label for="sn-plus" class="sidenote-toggle">⊕</label><input type="checkbox" id="sn-plus" class="sidenote-toggle" /><span class="sidenote">This is a sidenote without a number.</span></p>
...
&lt;span class="sidenote"&gt;&lt;!-- sidenote content --&gt;&lt;/span&gt;</code
		...
		<p>The symbol you could use to indicate a sidenote is up to you. What about this notebook. <label for="sn-symbol" class="sidenote-toggle">📝</label><input type="checkbox" id="sn-symbol" class="sidenote-toggle" /><span class="sidenote left">A notebook indicating a note. Aha. <br /> (if on a large screen, resize to mobile to see the emoji)</span></p>
		...
		<p>
			...
			<code>latex-dark</code> class to the <code class="language-html">&lt;body&gt;</code> element. This will force the document to be in dark mode.
		</p>
		<p>
		...
		<p>
			...
			<i>italic</i> is still around. <small>Small</small> text is for fine print. Your copy can also be <sub>subscripted</sub> and
		...
		<table>
			<caption>
				Example table taken from <a href="https://texdoc.net/texmf-dist/doc/latex/booktabs/booktabs.pdf">this paper</a> on how to produce quality tables with LaTeX.
			</caption>
			<thead>
				<tr>
		...
		<figure>
			<img src="https://images.unsplash.com/photo-1477346611705-65d1883cee1e?auto=format&fit=crop&w=1000&q=80" loading="lazy" alt="Mountain landscape" width="600" height="400" />
			<figcaption>
		...
		<p>It is best to break up long equations into smaller parts, but when this isn't possible, consider wrapping the overflowing element with a class of <code>scroll-wrapper</code> to allow scrolling on the x-axis. Large tables should also be wrapped with this class.</p>
		<p>Below is an example of a long equation with overflow scroll.</p>

		<h3 id="syntax-highlighting">Syntax Highlighting</h3>
		<p>If you need syntax highlighting for code, LaTeX.css provides a <a href="https://prismjs.com/">PrismJS</a> theme that immitates the <a href="https://github.com/gpoore/minted">minted</a> package for LaTeX. Add the following stylesheet and script:</p>
		<pre><code class="language-html"
				>&lt;link rel="stylesheet" href="https://latex.now.sh/prism/prism.css"&gt;
...
&lt;/pre&gt;</code
		...
		<div class="footnotes">
			<p id="fn1">
				1. From
				<a href="https://www.math.brown.edu/~res/MFS/handout8.pdf">https://www.math.brown.edu/~res/MFS/handout8.pdf</a>.
				<a href="#ref1" title="Jump back to footnote 1 in the text.">↩</a>
			</p>
			<p id="fn2">
				2. “Definition.” Merriam-Webster.com Dictionary, Merriam-Webster,
				<a href="https://www.merriam-webster.com/dictionary/definition">https://www.merriam-webster.com/dictionary/definition</a>. Accessed 18 May. 2020.
